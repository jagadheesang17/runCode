...
export class AdminLogin extends PlaywrightWrapper {
    ...
    protected async loadCookiesFromFile(role: string): Promise<boolean> {
        try {
            ...
            if (!cookieEnabledRoles.includes(role)) {
                console.log(`ℹ️ Skipping cookie auth for ${role} (only works for admin roles)`);
                return false;
            }
            ...
            console.log(`✅ Loaded ${cookies.length} cookies from cookies.json for ${role}`);
            return true;
        } catch (error: any) {
    ...
    protected async verifyCookiesValid(): Promise<boolean> {
        try {
            ...
            console.log("✅ Cookies are valid - authenticated");
            return true;
        } catch (error) {
    ...
    protected async regenerateCookies(role: string): Promise<boolean> {
        try {
            console.log(`🔄 Regenerating fresh cookies for ${role}...`);
            
            // Use the existing cookie setup function
            await setupCourseCreation();
            
            console.log(`✅ Fresh cookies regenerated for ${role}`);
            return true;
        } catch (error) {
            console.error(`❌ Failed to regenerate cookies for ${role}:`, error);
            return false;
        }
    }
