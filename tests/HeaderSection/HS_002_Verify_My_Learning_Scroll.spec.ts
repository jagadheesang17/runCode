...
test.describe('Header Section - My Learning Scroll Verification', () => {
  test('Verify page scrolls to My Learning section when clicked from header', async ({ page, context }) => {
    test.info().annotations.push(
      { type: 'Author', description: 'QA Automation Team' },
      { type: 'TestCase', description: 'HS_002_My_Learning_Scroll_Verification' },
      { type: 'Test Description', description: 'Login as learner, click My Learning, verify page scrolls' }
    );

    console.log('🔐 Starting My Learning Scroll Verification Test');
    ...
    // Step 2: Capture scroll position BEFORE clicking My Learning
    console.log('\n📝 Step 2: Capture scroll position before clicking My Learning');
    const beforeScroll = await page.evaluate(() => window.scrollY);
    console.log(`📏 Scroll position before click: ${beforeScroll}px`);
    
    // Step 3: Click My Learning button
    console.log('\n📝 Step 3: Click My Learning from header');
    const myLearningSelector = "//a//span[text()='My Learning']";
    await page.locator(myLearningSelector).click();
    console.log('✅ Clicked on My Learning');
    
    // Step 4: Wait for scroll to complete
    console.log('\n📝 Step 4: Wait for scroll animation to complete');
    await page.waitForTimeout(1000); // Wait for scroll animation
    
    // Step 5: Capture scroll position AFTER clicking My Learning
    console.log('\n📝 Step 5: Capture scroll position after clicking My Learning');
    ...
    console.log(`   ✅ Page scrolled to My Learning section`);
    console.log('\n🎯 TEST PASSED: Page scrolls when My Learning is clicked from header');
  });
});
