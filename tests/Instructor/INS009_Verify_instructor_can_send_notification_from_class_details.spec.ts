...
// Flattened to avoid describe-level issues in runner
  ...
  test(`Enroll a learner for the created VC`, async ({ adminHome, enrollHome }) => {
    test.info().annotations.push(
      { type: `Author`, description: `Automation` },
      { type: `TestCase`, description: `Enroll learner for scheduled VC` },
      { type: `Test Description`, description: `Enrolls a learner to ensure roster exists` }
    );
    ...
    await enrollHome.enterSearchUser("jagadish");
  ...
  //  await instructorHome.clickEnrollmentIcon(courseName);
    ...
    const ok = await instructorHome.sendNotificationByInstructor(subject, description);
