...
const vcCourseName = "VC_GoTo_" + FakerData.getCourseName();

// Sample GoToMeeting attendee URL
const GOTO_URL = "https://global.gotomeeting.com/join/123456789";

/**
 * Verify whether the instructor is able to launch the GoTo meeting
 */

test.describe("INS016_Verify_instructor_can_launch_GoTo_meeting", () => {
  ...
  test("Test 2: Create VC course (GoTo) and assign instructor", async ({ adminHome, createCourse, editCourse, instructorHome }) => {
    await adminHome.loadAndLogin("CUSTOMERADMIN");
    ...
    await createCourse.typeDescription("GoTo meeting launch");
    ...
    await createCourse.setAttendeeUrlForIndex(1, GOTO_URL);
    await createCourse.setPresenterUrlForIndex(1, GOTO_URL);
  ...
  test("Test 3: Instructor launches GoTo meeting", async ({ learnerHome, instructorHome }) => {
    ...
    if (!ok) throw new Error("Meeting screen not verified for GoToMeeting");
