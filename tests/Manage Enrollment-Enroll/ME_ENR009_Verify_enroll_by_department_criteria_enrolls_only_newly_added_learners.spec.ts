...
const courseName = "DeptCriteria_" + FakerData.getCourseName();
...
const batch1Users = Array.from({ length: 5 }, (_, i) => ({
    ...
    userId: `user_dept_batch1_${i + 1}_` + FakerData.getUserId()
}));
...
const batch2Users = Array.from({ length: 5 }, (_, i) => ({
    firstName: FakerData.getFirstName(),
    lastName: FakerData.getLastName(),
    userId: `user_dept_batch2_${i + 1}_` + FakerData.getUserId()
}));

test.describe(`ME_ENR009_Verify_enroll_by_department_criteria_enrolls_only_newly_added_learners`, () => {
    test.describe.configure({ mode: "serial" });

    test(`Test 1: Create E-learning course`, async ({ adminHome, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Jagadish` },
            { type: `TestCase`, description: `ME_ENR009_TC001 - Create course` },
            { type: `Test Description`, description: `Create E-learning course for department criteria enrollment testing` }
        );
    ...
    test(`Test 2: Create first batch of 5 users with department`, async ({ adminHome, createUser }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Jagadish` },
            { type: `TestCase`, description: `ME_ENR009_TC002 - Create batch 1 users with department` },
            { type: `Test Description`, description: `Create first batch of 5 users and assign them to a department` }
        );

        console.log(`🔄 Creating first batch of 5 users with department assignment...`);
        ...
        console.log(`✅ Batch 1: All 5 users created successfully with department: ${departmentName}`);
    });

    test(`Test 3: Assign same department to remaining Batch 1 users`, async ({ adminHome, createUser, page }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Jagadish` },
            { type: `TestCase`, description: `ME_ENR009_TC003 - Assign department to batch 1 users` },
            { type: `Test Description`, description: `Assign the same department to users 2-5 from batch 1` }
        );

        console.log(`🔄 Assigning department to remaining Batch 1 users (users 2-5)...`);
        ...
        console.log(`✅ Batch 1: All 5 users now have department: ${departmentName}`);
    });

    test(`Test 4: First enrollment - Enroll course by department criteria (Batch 1)`, async ({ adminHome, enrollHome, page }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Jagadish` },
            { type: `TestCase`, description: `ME_ENR009_TC004 - First enrollment via department criteria` },
            { type: `Test Description`, description: `Enroll course using department criteria - should enroll all 5 batch 1 users` }
        );

        console.log(`\n🔄 FIRST ENROLLMENT: Enrolling via Department criteria...`);
    ...
    test(`Test 5: Verify Batch 1 users are enrolled`, async ({ learnerHome, catalog }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Jagadish` },
            { type: `TestCase`, description: `ME_ENR009_TC005 - Verify batch 1 enrollment` },
    ...
    test(`Test 6: Create second batch of 5 NEW users with same department`, async ({ adminHome, createUser }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Jagadish` },
            { type: `TestCase`, description: `ME_ENR009_TC006 - Create batch 2 users with department` },
            { type: `Test Description`, description: `Create second batch of 5 NEW users and assign them to the same department` }
        );

        console.log(`\n🔄 Creating second batch of 5 NEW users with same department...`);
    ...
    test(`Test 7: Assign same department to all Batch 2 users`, async ({ adminHome, createUser, page }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Jagadish` },
            { type: `TestCase`, description: `ME_ENR009_TC007 - Assign department to batch 2 users` },
            { type: `Test Description`, description: `Assign the SAME department to all 5 NEW users from batch 2` }
        );

        console.log(`🔄 Assigning SAME department to all Batch 2 NEW users...`);
        
        const deptSearchSelector = "//label[text()='department']/following::div[@id='user-department']//input";

        for (let i = 0; i < batch2Users.length; i++) {
            const user = batch2Users[i];
            console.log(`   Assigning department to NEW User ${i + 1}/5: ${user.userId}`);
            ...
            await createUser.wait("minWait");
            await createUser.updateUser();
            console.log(`   ✅ NEW User ${i + 1} assigned to department`);
        }
        
        console.log(`✅ Batch 2: All 5 NEW users assigned to department: ${departmentName}`);
        console.log(`📊 Department now has: 10 total users (5 old + 5 new)`);
    });

    test(`Test 8: Second enrollment - Enroll SAME course by SAME department criteria`, async ({ adminHome, enrollHome, page }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Jagadish` },
            { type: `TestCase`, description: `ME_ENR009_TC008 - Second enrollment via same criteria` },
            { type: `Test Description`, description: `Re-enroll course using SAME department criteria - should enroll ONLY 5 newly added users` }
        );

        console.log(`\n🔄 SECOND ENROLLMENT: Re-enrolling via SAME Department criteria...`);
        ...
        // Use SAME Enroll By Criteria - Department
        const criteriaDropdowns = await page.locator("//label[text()='Enroll By Criteria']//following::button[contains(@data-id,'enroll-group')]");
        const firstDropdown = criteriaDropdowns.first();
        await firstDropdown.click();
        await enrollHome.wait("minWait");
        ...
        await page.locator("//span[text()='By Department']").click();
        console.log(`✅ Selected SAME criteria: By Department`);
        ...
        await page.locator(`//span[text()='${departmentName}']`).click();
        console.log(`✅ Selected SAME department: ${departmentName}`);
    ...
    test(`Test 9: Verify ONLY Batch 2 (NEW users) are enrolled after second enrollment`, async ({ learnerHome, catalog }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Jagadish` },
            { type: `TestCase`, description: `ME_ENR009_TC009 - Verify only new users enrolled` },
    ...
    test(`Test 10: Final summary and verification`, async ({ page }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Jagadish` },
            { type: `TestCase`, description: `ME_ENR009_TC010 - Final summary` },
            { type: `Test Description`, description: `Summary of incremental enrollment via department criteria verification` }
        );
        ...
        console.log(`   2. ✅ Created 5 users (Batch 1) with department assignment`);
        console.log(`   3. ✅ Assigned SAME department to all Batch 1 users`);
        console.log(`   4. ✅ First Enrollment: Enrolled by Department criteria`);
        ...
        console.log(`   7. ✅ Assigned SAME department to all Batch 2 users`);
        console.log(`   8. ✅ Second Enrollment: SAME criteria, SAME department`);
        ...
        console.log(`   ✅ Batch 2: 5 NEW users added to same department`);
        console.log(`   ✅ Second enrollment enrolled ONLY newly added users`);
        console.log(`   ✅ Old users (Batch 1) were NOT re-enrolled`);
        console.log(`\n🎯 TEST RESULT: Enroll By Department Criteria correctly enrolls ONLY newly added learners`);
        console.log(`🎯 BUSINESS RULE: System tracks previous enrollments and excludes already enrolled users`);
        console.log(`🎯 CONFIRMED: Incremental enrollment functionality working as expected for Department criteria`);
        console.log(`✅ ========================================\n`);
    });
});
