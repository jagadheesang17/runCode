...
const courseName = "StatusUpdate_" + FakerData.getCourseName();
const description = FakerData.getDescription();
const learner = credentials.LEARNERUSERNAME.username;

test.describe(`ME_VUS011_Verify_admin_can_update_status_from_enrolled_to_completed`, () => {
    test.describe.configure({ mode: "serial" });

    test(`Test 1: Create E-learning course`, async ({ adminHome, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Jagadish` },
            { type: `TestCase`, description: `ME_VUS011_TC001 - Create course` },
            { type: `Test Description`, description: `Create E-learning course for status update testing` }
        );
    ...
    test(`Test 2: Enroll learner to the course`, async ({ adminHome, enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Jagadish` },
            { type: `TestCase`, description: `ME_VUS011_TC002 - Enroll learner` },
    ...
    test(`Test 3: Verify current status is Enrolled`, async ({ adminHome, enrollHome, page }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Jagadish` },
            { type: `TestCase`, description: `ME_VUS011_TC003 - Verify Enrolled status` },
    ...
    test(`Test 4: Update status from Enrolled to Completed`, async ({ adminHome, enrollHome, page }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Jagadish` },
            { type: `TestCase`, description: `ME_VUS011_TC004 - Update to Completed` },
            { type: `Test Description`, description: `Update enrollment status from Enrolled to Completed` }
        );

        console.log(`🔄 Updating status from Enrolled to Completed...`);
        await adminHome.loadAndLogin("CUSTOMERADMIN");
        await adminHome.menuButton();
        await adminHome.clickEnrollmentMenu();
        await adminHome.clickviewUpdateStatusCourseTp();
        ...
        // Select Completed
        const completedOption = page.locator(`//button[contains(@data-id,'enrollment-action')]/following::span[text()='Completed']`).first();
    ...
    test(`Test 5: Verify status updated to Completed`, async ({ adminHome, enrollHome, page }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Jagadish` },
            { type: `TestCase`, description: `ME_VUS011_TC005 - Verify Completed status` },
            { type: `Test Description`, description: `Verify enrollment status has been updated to Completed` }
        );

        console.log(`🔄 Verifying status updated to Completed...`);
        await adminHome.loadAndLogin("CUSTOMERADMIN");
        await adminHome.menuButton();
        await adminHome.clickEnrollmentMenu();
        await adminHome.clickviewUpdateStatusCourseTp();
        ...
        if (updatedStatus.includes('Completed')) {
            console.log(`   ✅ PASS: Status successfully updated from Enrolled to Completed`);
        } else {
            console.log(`   ⚠️ WARNING: Status shows "${updatedStatus}" (expected Completed)`);
        }
    });
});
