...
test.describe(`Verify admin can update enrollment type from Mandatory to Optional - Course/TP`, () => {
    ...
    test(`Setup - Create E-learning course and enroll 3 learners as Mandatory & verify`, async ({ adminHome, enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Kathir A` },
            { type: `TestCase`, description: `ENR_VUS_004 - Setup E-learning course with Mandatory enrollment` },
            { type: `Test Description`, description: `Create E-learning course via API, enroll 3 learners as Mandatory and verify in View/Update Status` }
        );

        // Create E-learning course via API
        console.log(`🔄 Creating E-learning course: ${courseName}`);
        ...
        // Enroll all 3 users as Mandatory
    ...
    test(`Verify Mandatory enrollment type from learner side - All 3 users`, async ({ catalog, learnerHome, dashboard, createUser }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Kathir A` },
            { type: `TestCase`, description: `ENR_VUS_004 - Verify learner side Mandatory enrollment` },
            { type: `Test Description`, description: `All 3 users verify Mandatory text is visible in My Learning for Course` }
        );
    ...
    test(`Admin updates User 1 & User 3 from Mandatory to Optional & verify`, async ({ adminHome, enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Kathir A` },
            { type: `TestCase`, description: `ENR_VUS_004 - Update enrollment type to Optional` },
    ...
    test(`Verify enrollment type from learner side - All 3 users`, async ({ catalog, learnerHome, dashboard, createUser }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Kathir A` },
            { type: `TestCase`, description: `ENR_VUS_004 - Verify learner side enrollment type` },
