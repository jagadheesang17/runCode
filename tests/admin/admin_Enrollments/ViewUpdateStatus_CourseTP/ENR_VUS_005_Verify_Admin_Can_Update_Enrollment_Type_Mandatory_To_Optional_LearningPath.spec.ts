...
test.describe(`Verify admin can update enrollment type from Mandatory to Optional - Learning Path`, () => {
    ...
    test(`Setup - Create ILT multi-instance course, create learning path, assign course & verify`, async ({ adminHome,editCourse ,enrollHome, learningPath, contentHome, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Kathir A` },
            { type: `TestCase`, description: `ENR_VUS_LP_001 - Setup ILT multi-instance course and learning path` },
            { type: `Test Description`, description: `Create ILT multi-instance course via API, create learning path, assign course, enroll 3 learners as Mandatory and verify in View/Update Status` }
        );
    ...
    test(`Verify Mandatory enrollment type from learner side - All 3 users`, async ({ catalog, learnerHome, dashboard, createUser }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Kathir A` },
            { type: `TestCase`, description: `ENR_VUS_LP_001 - Verify learner side Mandatory enrollment` },
            { type: `Test Description`, description: `All 3 users verify Mandatory text is visible in My Learning for Learning Path` }
        );
        
        // User 1: Should see Mandatory (Mandatory text visible)
        console.log(`🔄 User 1 (${users[0].username}) - Verifying Mandatory enrollment`);
        for (let i = 0; i < users.length; i++) {
    ...
    test(`Admin updates User 1 & User 3 from Mandatory to Optional & verify`, async ({ adminHome, enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Kathir A` },
            { type: `TestCase`, description: `ENR_VUS_LP_001 - Update enrollment type to Optional` },
    ...
    test(`Verify enrollment type from learner side - All 3 users`, async ({ catalog, learnerHome, dashboard, createUser }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Kathir A` },
            { type: `TestCase`, description: `ENR_VUS_LP_001 - Verify learner side enrollment type` },
