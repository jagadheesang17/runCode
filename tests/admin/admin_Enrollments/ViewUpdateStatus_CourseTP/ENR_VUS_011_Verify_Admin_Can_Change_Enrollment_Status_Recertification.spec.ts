...
const certificationName = "Optical Circuit Copy";// FakerData.getCourseName();
const certificationCourse = "Auxiliary Bus Program";// FakerData.getCourseName();
const reCertificationCourse =  "Cross-platform Pixel Transmit";//FakerData.getCourseName();
...
test.describe(`Verify admin can change enrollment status for Recertification`, () => {
    ...
    test(`Test 2: Verify learners can see certification and trigger recertification flow`, async ({ learnerHome, createUser, dashboard ,catalog}) => {
        ...
        for (let i = 0; i < Math.min(users.length, 3); i++) {
            ...
            await dashboard.clickRecertifyIcon(certificationName);
