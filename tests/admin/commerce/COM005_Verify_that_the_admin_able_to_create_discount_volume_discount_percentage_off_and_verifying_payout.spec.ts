...
const discountName = FakerData.getCategory() + " " + "Discounts";
const description = FakerData.getDescription();
const price = FakerData.getPrice();
const code = "DIS-" + generateCode();
let dicountValue: any
const portalName = getRandomItemFromFile("../data/newDomain.json");
test.describe.configure({ mode: "serial" });
test(`Verify that the admin able to create volume discount with Percentage Off`, async ({ adminHome, commercehome, createCourse, discount, SurveyAssessment }) => {
    ...
    dicountValue = await discount.setDiscountRules("Percentage Off", "Volume");
...
test(`Creating paid EL course and adding discount`, async ({ createCourse, adminHome, editCourse }) => {
    ...
    await createCourse.selectDomainOption(portalName);
    await createCourse.uploadvideo();
    await createCourse.uploadThumbnailfromCustomGallery(); 
    ...
    await editCourse.selectDiscountOption("Minimum");
