...
const discountName = FakerData.getTagNames() + " " + "PercentageDiscount";
const description = FakerData.getDescription();
const price = "100";
const code = "DIS-" + generateCode();
let discountValue: any;

test.describe.serial(`COM022 - Verify percentage off discount calculation with $100 course`, () => {

test(`Step 1: Create linear discount with 10% off`, async ({ adminHome, commercehome, createCourse, discount, SurveyAssessment }) => {
    test.info().annotations.push(
        { type: `Author`, description: `Divya` },
        { type: `TestCase`, description: `COM022 - Verify 10% discount on $100 course` },
        { type: `Test Description`, description: `Verify 10% discount deducts $10 from $100 course, resulting in $90 subtotal` }
    );
    ...
    discountValue = await discount.setDiscountRules("Percentage Off", "US Dollar", "10");
    await discount.discountCriteria("Domain");
    console.log(`✅ Step 1 Passed: Created linear discount with ${discountValue}% off`);
...
test(`Step 3: Create order and verify discount calculation`, async ({ adminHome, editCourse, enrollHome, costCenter }) => {
    ...
    // Calculate expected discount: 10% of $100 = $10
    const expectedDiscount = (parseFloat(price) * discountValue) / 100;
    expect(discountAmount).toBe(expectedDiscount);
    console.log(`✅ Discount calculation verified: ${discountValue}% of $${price} = $${expectedDiscount}`);
