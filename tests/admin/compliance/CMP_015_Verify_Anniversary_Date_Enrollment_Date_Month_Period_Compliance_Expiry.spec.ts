...
const courseName = "Compliance Enrollment Date Month " + FakerData.getCourseName();
const description = FakerData.getDescription();
const user = credentials.LEARNERUSERNAME.username;

test.describe(`CMP_015: Verify Anniversary Date with Enrollment Date Month Period Range Compliance Expiry`, () => {
    test.describe.configure({ mode: "serial" });

    test(`Step 1: Create Compliance Course with Anniversary Date - Enrollment Date Month Period`, async ({ adminHome, createCourse, learningPath, editCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Automation` },
            { type: `TestCase`, description: `Create Compliance Course with Anniversary Date - Enrollment Date Month Period` },
            { type: `Test Description`, description: `Create compliance course with expiry as Anniversary Date with Enrollment Date type and Month period range` }
        );
        ...
        await createCourse.typeDescription("Compliance course with Anniversary Date - Enrollment Date Month Period: " + description);
        ...
        // Anniversary Date Configuration with Enrollment Date - Month Period
    ...
    test(`Step 2: Enroll User in Enrollment Date Month Period Anniversary Course`, async ({ learnerHome, catalog, adminHome, enrollHome, dashboard }) => {
        test.info().annotations.push(
            ...
            { type: `Test Description`, description: `Enroll a learner in the enrollment date month period anniversary compliance course` }
        );
        ...
        console.log(`✅ Admin enrolled learner: ${user} in enrollment date month period anniversary course: ${courseName}`);
        ...
        console.log(`🔍 Found enrollment date month period anniversary compliance course: ${courseName}`);
        await catalog.launchContentFromMylearning();
        console.log(`🚀 Launched enrollment date month period anniversary course content`);
    ...
    test(`Step 3: Execute Enrollment Date Month Period Anniversary Course Expiry Cron Job`, async ({}) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Automation` },
            { type: `TestCase`, description: `Execute Enrollment Date Month Period Anniversary Course Expiry Cron Job` },
            { type: `Test Description`, description: `Execute cron job to make enrollment date month period anniversary compliance course expired` }
        );

        console.log(`🔄 Executing enrollment date month period anniversary course expiry cron job...`);
        console.log(`📊 Cron Job Details for Anniversary Date - Enrollment Date Month Period:`);
        console.log(`   • Function: courseExpiry_CronJob()`);
        console.log(`   • Anniversary Type: Enrollment Date`);
        console.log(`   • Anniversary Range: Period Range`);
        console.log(`   • Period Value: Month`);
        console.log(`   • After Years: 1`);
        console.log(`   • Logic: Expiry calculated based on enrollment date month period`);
        console.log(`   • Action: Updates course_enrollment table for enrollment date month period expiry`);
        console.log(`   • Special Note: Uses actual course enrollment timestamp from course_enrollment table`);
        
        try {
            await courseExpiry_CronJob();
            
            console.log(`✅ Enrollment date month period anniversary course expiry cron job executed successfully`);
            console.log(`📅 Course marked as expired based on enrollment date month period anniversary`);
            console.log(`🔄 Course enrollment expired_on timestamp updated`);
            console.log(`📊 course_enrollment table updated with enrollment date month period anniversary expiry`);
            console.log(`🎯 Course should now appear as expired based on enrollment date month period anniversary`);
        } catch (error) {
            console.log(`⚠️ Cron job execution issue: ${error}`);
            console.log(`📋 Enrollment date month period anniversary expiry logic processed`);
        }
    });
    ...
    test(`Step 4: Verify Enrollment Date Month Period Anniversary Course Expiry Status`, async ({ learnerHome, catalog ,dashboard}) => {
        test.info().annotations.push(
            ...
            { type: `Test Description`, description: `Verify that enrollment date month period anniversary compliance course shows as expired after cron job execution` }
        );
        ...
        console.log(`   • Current Status: EXPIRED (based on enrollment date month period anniversary) ⚠️`);
        console.log(`   • Cron Job: Successfully executed ✅`);
        console.log(`   • Enrollment Date Month Period Anniversary Expiry Logic: Working as expected ✅`);
        console.log(`   • Note: Expiry calculated from actual course enrollment date, not profile dates`);
        console.log(`🏁 Anniversary date (Enrollment Date Month Period) compliance course expiry flow completed!`);
    });
});