...
const courseName = "ELearning_BulkEnroll_" + FakerData.getCourseName();
...
test.describe(`BLK_ENR_002: Verify Enrolled Status for E-Learning Course via Bulk Upload`, async () => {
    test.describe.configure({ mode: "serial" });

    test(`Step 0: Set Allow Excel Configuration to 0`, async ({ adminHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_002_Step0: Configuration Setup` },
            { type: `Test Description`, description: `Configure allow_excel setting to 0 for E-learning bulk upload testing` }
        );
    ...
    test(`Step 1: Create Single Instance E-Learning Course`, async ({ adminHome, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_002_Step1: E-Learning Course Creation` },
            { type: `Test Description`, description: `Create E-learning course for bulk enrollment testing` }
        );
    ...
    test(`Step 2: Create Test Users for Bulk Enrollment`, async ({ adminHome, createUser }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_002_Step2: User Creation` },
            { type: `Test Description`, description: `Create users for bulk enrollment in E-learning course` }
        );
    ...
    test(`Step 3: Perform Bulk Enrollment Upload for E-Learning Course`, async ({ adminHome, enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_002_Step3: Bulk Upload` },
            { type: `Test Description`, description: `Upload bulk enrollment file for E-learning course and verify` }
        );
        ...
        // Data lines for E-learning course
        lines.push(`${testUsers[0].username}|${courseCode}|Enrolled|${formattedDate}|||English|N|N`);
        lines.push(`${testUsers[1].username}|${courseCode}|Enrolled|${formattedDate}|||English|N|N`);
        
        // Join with Unix line endings (\n only)
        const fileContent = lines.join('\n');
        
        // Write both .csv and .psv versions
        const csvFilePath = path.join(process.cwd(), 'data', 'bulk_enrollment_elearning_test.csv');
        const psvFilePath = path.join(process.cwd(), 'data', 'bulk_enrollment_elearning_test.psv');
        ...
        console.log(`📄 E-Learning Pipe-delimited files created:`);
    ...
    test(`Step 4: Verify Enrollment Status in E-Learning Course`, async ({ adminHome, createCourse, enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
