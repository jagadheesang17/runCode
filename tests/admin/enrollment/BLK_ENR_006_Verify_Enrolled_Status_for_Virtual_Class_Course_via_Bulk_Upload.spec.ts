...
const courseName = "VC_BulkEnroll_" + FakerData.getCourseName();
...
test.describe(`BLK_ENR_006: Verify Enrolled Status for Virtual Class Course via Bulk Upload`, async () => {
    test.describe.configure({ mode: "serial" });

    test(`Step 0: Set Allow Excel Configuration to 0`, async ({ adminHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_006_Step0: Configuration Setup` },
            { type: `Test Description`, description: `Configure allow_excel setting to 0 for Virtual Class bulk enrollment testing` }
        );
    ...
    test(`Step 1: Create Future Virtual Class Course with Instance`, async ({ adminHome, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_006_Step1: Virtual Class Course Creation` },
            { type: `Test Description`, description: `Create Virtual Class course with future instance for bulk enrollment testing` }
        );
    ...
    test(`Step 2: Create Test Users for Bulk Enrollment`, async ({ adminHome, createUser }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_006_Step2: User Creation` },
            { type: `Test Description`, description: `Create users for bulk enrollment in Virtual Class course` }
        );
    ...
    test(`Step 3: Perform Bulk Enrollment Upload for Virtual Class Course`, async ({ adminHome, enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_006_Step3: Bulk Upload` },
            { type: `Test Description`, description: `Upload bulk enrollment file for Virtual Class course and verify` }
        );
        ...
        // Data lines for Virtual Class course - regdate uses current date (enrollment date)
        lines.push(`${testUsers[0].username}|${instanceCode}|Enrolled|${regDate}|||English|N|N`);
        lines.push(`${testUsers[1].username}|${instanceCode}|Enrolled|${regDate}|||English|N|N`);
        
        // Join with Unix line endings (\n only)
        const fileContent = lines.join('\n');
        
        // Write both .csv and .psv versions
        const csvFilePath = path.join(process.cwd(), 'data', 'bulk_enrollment_virtual_class.csv');
        const psvFilePath = path.join(process.cwd(), 'data', 'bulk_enrollment_virtual_class.psv');
        ...
        console.log(`📄 Virtual Class Pipe-delimited enrollment files created:`);
    ...
    test(`Step 4: Verify Enrollment Status in Virtual Class Course`, async ({ adminHome, createCourse, enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_006_Step4: Verification` },
            { type: `Test Description`, description: `Verify users are enrolled in Virtual Class course with correct status` }
        );
        ...
        // Verify enrollment status appears in the enrollment list
        await enrollHome.page.waitForSelector('text=Enrolled', { timeout: 10000 });
        
        console.log(`✅ Virtual Class course enrollment verification completed for users: ${testUsers[0].username}, ${testUsers[1].username}`);
        console.log(`📊 Status: Enrolled (Virtual Class)`);
        console.log(`🎯 Instance Code: ${instanceCode}`);
    });

    test(`Step 5: Verify Users in My Learning with Virtual Class Course`, async ({ learnerHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_006_Step5: Learner Verification` },
            { type: `Test Description`, description: `Verify enrolled Virtual Class course appears in learner's My Learning section` }
        );

        // Login as first test user to verify enrollment in My Learning
        ...
        await learnerHome.page.waitForSelector('text=Enrolled', { timeout: 5000 });
        
        console.log(`✅ Virtual Class course "${courseName}" verified as enrolled in learner's My Learning`);
        console.log(`🎯 Future VC instance ready for attendance`);
    });
