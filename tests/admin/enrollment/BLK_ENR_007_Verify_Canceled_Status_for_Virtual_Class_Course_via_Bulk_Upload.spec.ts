...
const courseName = "VC_BulkCancel_" + FakerData.getCourseName();
...
test.describe(`BLK_ENR_007: Verify Canceled Status for Virtual Class Course via Bulk Upload`, async () => {
    test.describe.configure({ mode: "serial" });

    test(`Step 0: Set Allow Excel Configuration to 0`, async ({ adminHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_007_Step0: Configuration Setup` },
            { type: `Test Description`, description: `Configure allow_excel setting to 0 for Virtual Class bulk cancellation testing` }
        );
    ...
    test(`Step 1: Create Future Virtual Class Course with Instance`, async ({ adminHome, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_007_Step1: Virtual Class Course Creation` },
            { type: `Test Description`, description: `Create Virtual Class course with future instance for bulk cancellation testing` }
        );
    ...
    test(`Step 2: Create Test Users for Bulk Cancellation`, async ({ adminHome, createUser }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_007_Step2: User Creation` },
            { type: `Test Description`, description: `Create users for bulk cancellation in Virtual Class course` }
        );
    ...
    test(`Step 3: Perform Bulk Cancellation Upload for Virtual Class Course`, async ({ adminHome, enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_007_Step3: Bulk Cancellation Upload` },
            { type: `Test Description`, description: `Upload bulk cancellation file for Virtual Class course and verify` }
        );
        ...
        // Data lines for Virtual Class course - regdate uses current date (enrollment date) for cancellation
        ...
        const csvFilePath = path.join(process.cwd(), 'data', 'bulk_cancellation_virtual_class.csv');
        const psvFilePath = path.join(process.cwd(), 'data', 'bulk_cancellation_virtual_class.psv');
        ...
        console.log(`📄 Virtual Class Pipe-delimited cancellation files created:`);
    ...
    test(`Step 4: Verify Canceled Status in Virtual Class Course`, async ({ adminHome, createCourse, enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_007_Step4: Verification` },
            { type: `Test Description`, description: `Verify users are canceled in Virtual Class course with correct status` }
        );
        ...
        // Verify both users are canceled
        ...
        console.log(`✅ Virtual Class course cancellation verification completed for users: ${testUsers[0].username}, ${testUsers[1].username}`);
        console.log(`📊 Status: Canceled (Virtual Class)`);
        console.log(`🎯 Instance Code: ${instanceCode}`);
    });

    test(`Step 5: Verify Users in My Learning with Canceled Virtual Class Course`, async ({ learnerHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_007_Step5: Learner Verification` },
            { type: `Test Description`, description: `Verify canceled Virtual Class course appears in learner's My Learning section` }
        );
        ...
        console.log(`✅ Virtual Class course "${courseName}" verified as canceled in learner's My Learning`);
        console.log(`🎯 VC instance canceled - no future attendance required`);
    });
