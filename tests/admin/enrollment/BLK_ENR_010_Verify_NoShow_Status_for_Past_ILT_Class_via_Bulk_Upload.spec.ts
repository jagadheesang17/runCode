...
const courseName = "ILT_BulkNoShow_" + FakerData.getCourseName();
...
test.describe(`BLK_ENR_010: Verify No Show Status for Past ILT Class via Bulk Upload`, async () => {
    test.describe.configure({ mode: "serial" });

    test(`Step 0: Set Allow Excel Configuration to 0`, async ({ adminHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_010_Step0: Configuration Setup` },
            { type: `Test Description`, description: `Configure allow_excel setting to 0 for Past ILT bulk no show testing` }
        );
    ...
    test(`Step 1: Create Past ILT Class with Instance`, async ({ adminHome, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_010_Step1: Past ILT Course Creation` },
            { type: `Test Description`, description: `Create ILT course with past session for bulk no show testing` }
        );
    ...
    test(`Step 2: Create Test Users for Bulk No Show`, async ({ adminHome, createUser }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_010_Step2: User Creation` },
            { type: `Test Description`, description: `Create users for bulk no show in Past ILT course` }
        );
    ...
    test(`Step 3: Perform Bulk No Show Upload for Past ILT Class`, async ({ adminHome, enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_010_Step3: Bulk No Show Upload` },
            { type: `Test Description`, description: `Upload bulk no show file for Past ILT course and verify` }
        );
        ...
        // Data lines for Past ILT course - No Show status, no completion date or score
        lines.push(`${testUsers[0].username}|${instanceCode}|No Show|${regDate}|||English|N|N`);
        lines.push(`${testUsers[1].username}|${instanceCode}|No Show|${regDate}|||English|N|N`);
        
        // Join with Unix line endings (\n only)
        const fileContent = lines.join('\n');
        
        // Write both .csv and .psv versions
        const csvFilePath = path.join(process.cwd(), 'data', 'bulk_noshow_past_ilt.csv');
        const psvFilePath = path.join(process.cwd(), 'data', 'bulk_noshow_past_ilt.psv');
        ...
        console.log(`📄 Past ILT Pipe-delimited no show files created:`);
    ...
    test(`Step 4: Verify No Show Status in Past ILT Course`, async ({ adminHome, createCourse, enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_010_Step4: Verification` },
            { type: `Test Description`, description: `Verify users are marked as No Show in Past ILT course` }
        );
        ...
        // Verify both users are marked as No Show
        ...
        // Verify No Show status appears in the enrollment list
        await enrollHome.page.waitForSelector('text=No Show', { timeout: 10000 });
        
        console.log(`✅ Past ILT course no show verification completed for users: ${testUsers[0].username}, ${testUsers[1].username}`);
        console.log(`📊 Status: No Show (Past ILT no attendance)`);
        console.log(`🎯 Instance Code: ${instanceCode}`);
    });

    test(`Step 5: Verify Users in My Learning with No Show Past ILT Course`, async ({ learnerHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `QA Team` },
            { type: `TestCase`, description: `BLK_ENR_010_Step5: Learner Verification` },
            { type: `Test Description`, description: `Verify no show Past ILT course appears in learner's My Learning section` }
        );

        // Login as first test user to verify no show status in My Learning
        await learnerHome.basicLogin(testUsers[0].username, "Default");
        await learnerHome.clickMyLearning();
        
        // Verify course appears with No Show status
        await learnerHome.page.waitForSelector(`text=${courseName}`, { timeout: 10000 });
        await learnerHome.page.waitForSelector('text=No Show', { timeout: 5000 });
        
        console.log(`✅ Past ILT course "${courseName}" verified as No Show in learner's My Learning`);
        console.log(`🎯 Past ILT marked as No Show`);
    });
