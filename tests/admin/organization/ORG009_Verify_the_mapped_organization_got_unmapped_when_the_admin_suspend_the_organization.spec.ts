...
const OrgName = FakerData.getOrganizationName()+"org"
 let ogtypeValue:any;
...
await adminHome.loadAndLogin("CUSTOMERADMIN");
  ...
  await organization.clickSuspendIcon();
    await organization.verifyTheMappedUserGotUnMappedWhenSuspendingOrg(OrgName);
...
test(`Verify the organization after suspended by the admin`, async ({ adminHome, createUser ,createCourse}) => {
    test.info().annotations.push(
        { type: `Author`, description: `Tamilvanan` },
        { type: `TestCase`, description: `Verify that user address validation functionality working as expected` },
        { type: `Test Description`, description: `Creating the user and verifying user address validation functionality working as expected` }
    ...
    const data = await readDataFromCSV(csvFilePath);
        ...
        try {
           await createUser.searchAndSelect("organization",OrgName);
            console.log("WARNING: Suspended organization not allowed to set as parent organization");
        } catch (error) {
            console.log("EXPECTED:Suspended organization is successfully unmapped from the user  - " + error);
        }
