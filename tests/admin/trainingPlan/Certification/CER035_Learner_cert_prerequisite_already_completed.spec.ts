...
let tag: any
test.describe(`Verify_if_the_learner_already_completed_the_course_attached_as_certification_prerequisite_and_completed_status_reflected_when_the_learner_enrolls_certification_without_warning_popup`, async () => {
 ...
 test(`learner who complete the prerequisite course before attaching it to certification`, async ({ learnerHome, catalog }) => {
    ...
    test(`Attaching the main course and pre-requisite to the certification`, async ({ adminHome, learningPath, createCourse, editCourse, enrollHome, catalog }) => {
     ...
     test(`Verify the reflection of prereq's completed status when learner enrolls in certification without warning popup`, async ({ learnerHome, catalog, dashboard }) => {
            test.info().annotations.push(
