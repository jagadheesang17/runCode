...
test(`Completing the equivalence course as by standalaone`, async ({ learnerHome, catalog }) => {
 ...
 test(`Attaching the main course with equivalence to the certification`, async ({ adminHome, learningPath, createCourse, createUser }) => {
        test.info().annotations.push(
...
test(`Verify the TP status when the learner already completed the equivalence by standalone which is attached to the certification`, async ({ learnerHome, catalog, dashboard }) => {
    test.info().annotations.push(
        { type: `Author`, description: `Tamilvanan` },
        { type: `TestCase`, description: `Verifying learner side that equivalence functionality works correctly` },
        { type: `Test Description`, description: `Verifying learner side that equivalence functionality works correctly` }
