...
test.describe(`TP091_Verify_compliance_certification_recertification_with_anniversary_birth_date_quarter_and_days_from_hire`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_RECERT_BirthDate_Quarter_DaysFromHire_" + FakerData.getCourseName());
    test(`Create compliance certification with Birth Date Quarter and Recertification Complete By Days from hire`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Birth Date Quarter expiry and Days from hire rule` },
            { type: `Test Description`, description: `Create compliance certification with Birth Date Period Quarter expiry and recertification complete by Days from hire` }
        );

        const newData = {
            TP091: titleAnniversaryDate
        };
        ...
        console.log(`🔄 Setting expiration with Anniversary Date - Birth Date, Period Range, Quarter`);
        ...
        console.log(`🔄 Setting recertification complete by rule: Days from hire`);
        await learningPath.clickAndSelectRecertCompleteByRule("Days from hire");
