...
test.describe(`TP098_Verify_compliance_certification_recertification_with_anniversary_enrollment_date_year_and_complete_by_date`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_RECERT_EnrollDate_Year_CompleteByDate_" + FakerData.getCourseName());
    test(`Create compliance certification with Enrollment Date Year and Recertification Complete By Date`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Enrollment Date Year expiry and Complete By Date rule` },
            { type: `Test Description`, description: `Create compliance certification with Enrollment Date Period Year expiry and recertification complete by Date` }
        );

        const newData = {
            TP098: titleAnniversaryDate
        };
