...
test.describe(`TP059_Verify_certification_with_optional_course_and_enforce_sequence.spec.ts`, async () => {
    test.describe.configure({ mode: "serial" });

    test(`Create 3 courses through API for certification`, async () => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `TP059 - Create 3 courses via API` },
    ...
    test(`Create certification with optional course and enforce sequence`, async ({ adminHome, learningPath }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `TP059 - Create certification with optional course and enforce sequence` },
            { type: `Test Description`, description: `Create certification, add 3 courses, make one optional, set completion required to 1, enable enforce sequence` }
        );
        ...
        console.log(`✅ Successfully created certification with optional course and enforce sequence: ${certificationName}`);
        console.log(`   Course 1 (Required): ${courseName1}`);
        console.log(`   Course 2 (Required): ${courseName2}`);
        console.log(`   Course 3 (Optional): ${courseName3}`);
        console.log(`   Completion Required: 1`);
        console.log(`   Enforce Sequence: Enabled`);
    });

    test(`Learner enrolls in certification and verifies enforce sequence`, async ({ learnerHome, catalog, page }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `TP059 - Verify learner enrollment with enforce sequence` },
            { type: `Test Description`, description: `Verify enforce sequence prevents skipping courses and test optional course enrollment` }
        );

        console.log(`🔄 Learner logging in and enrolling in certification`);
