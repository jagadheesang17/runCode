...
test.describe(`TPC091_Verify_compliance_certification_recertification_expiry_with_anniversary_birth_date_period_quarter`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_RECERT_BirthDate_Quarter_" + FakerData.getCourseName());
    test(`Create compliance certification with Anniversary Date Birth Date Quarter and Recertification expiry`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Anniversary Date Birth Date Quarter and Recertification expiry` },
            { type: `Test Description`, description: `Create compliance certification with expiry based on Birth Date, Period Range Quarter and recertification expiry` }
        );

        const newData = {
            TPC091: titleAnniversaryDate
        };
        ...
        console.log(`🔄 Setting recertification expiry with Anniversary Date - Birth Date, Period Range, Quarter`);
