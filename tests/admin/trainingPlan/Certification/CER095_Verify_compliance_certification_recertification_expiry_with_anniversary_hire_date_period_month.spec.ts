...
test.describe(`TPC093_Verify_compliance_certification_recertification_expiry_with_anniversary_hire_date_period_month`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_RECERT_HireDate_Month_" + FakerData.getCourseName());
    test(`Create compliance certification with Anniversary Date Hire Date Month and Recertification expiry`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Anniversary Date Hire Date Month and Recertification expiry` },
            { type: `Test Description`, description: `Create compliance certification with expiry based on Hire Date, Period Range Month and recertification expiry` }
        );

        const newData = {
            TPC093: titleAnniversaryDate
        };
        ...
        console.log(`🔄 Setting recertification expiry with Anniversary Date - Hire Date, Period Range, Month`);
