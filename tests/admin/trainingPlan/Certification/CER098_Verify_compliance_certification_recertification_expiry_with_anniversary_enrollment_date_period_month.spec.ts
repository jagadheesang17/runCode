...
test.describe(`TPC096_Verify_compliance_certification_recertification_expiry_with_anniversary_enrollment_date_period_month`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_RECERT_EnrollmentDate_Month_" + FakerData.getCourseName());
    test(`Create compliance certification with Anniversary Date Enrollment Date Month and Recertification expiry`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Anniversary Date Enrollment Date Month and Recertification expiry` },
            { type: `Test Description`, description: `Create compliance certification with expiry based on Enrollment Date, Period Range Month and recertification expiry` }
        );

        const newData = {
            TPC096: titleAnniversaryDate
        };
        ...
        console.log(`🔄 Setting recertification expiry with Anniversary Date - Enrollment Date, Period Range, Month`);
        await learningPath.clickReCertExpiresDropdown();
        await learningPath.clickReCertExpiresButtonWithType("Anniversary Date", {
            anniversaryType: "Enrollment",
            anniversaryRange: "Period Range",
            period: "Month",
            periodYears: "1"
        });
