...
test.describe(`TPC098_Verify_compliance_certification_recertification_expiry_with_anniversary_enrollment_date_period_year`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_RECERT_EnrollmentDate_Year_" + FakerData.getCourseName());
    test(`Create compliance certification with Anniversary Date Enrollment Date Year and Recertification expiry`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Anniversary Date Enrollment Date Year and Recertification expiry` },
            { type: `Test Description`, description: `Create compliance certification with expiry based on Enrollment Date, Period Range Year and recertification expiry` }
        );

        const newData = {
            TPC098: titleAnniversaryDate
        };
        ...
        console.log(`🔄 Setting expiration with Anniversary Date - Enrollment Date, Period Range, Year`);
        ...
        console.log(`🔄 Setting recertification expiry with Anniversary Date - Enrollment Date, Period Range, Year`);
