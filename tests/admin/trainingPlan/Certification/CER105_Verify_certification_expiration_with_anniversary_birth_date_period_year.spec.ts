...
test.describe(`TP071_Verify_certification_expiration_with_anniversary_birth_date_period_year`, async () => {
    ...
    const titleAnniversaryDate = ("CERT_BirthDate_Year_" + FakerData.getCourseName());
    test(`Create certification with Anniversary Date - Birth Date, Period Range Year`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create certification with Anniversary Date - Birth Date, Period Range Year` },
            { type: `Test Description`, description: `Create certification with expiry based on Birth Date, Period Range, Year, 1 year` }
        );

        const newData = {
            TP071: titleAnniversaryDate
        };
