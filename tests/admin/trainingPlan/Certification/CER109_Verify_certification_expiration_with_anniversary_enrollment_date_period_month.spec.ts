...
test.describe(`TP075_Verify_certification_expiration_with_anniversary_enrollment_date_period_month`, async () => {
    ...
    const titleAnniversaryDate = ("CERT_EnrollmentDate_Month_" + FakerData.getCourseName());
    test(`Create certification with Anniversary Date - Enrollment Date, Period Range Month`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create certification with Anniversary Date - Enrollment Date, Period Range Month` },
            { type: `Test Description`, description: `Create certification with expiry based on Enrollment Date, Period Range, Month, 1 year` }
        );

        const newData = {
            TP075: titleAnniversaryDate
        };
