...
test.describe(`TP076_Verify_certification_expiration_with_anniversary_enrollment_date_period_quarter`, async () => {
    ...
    const titleAnniversaryDate = ("CERT_EnrollmentDate_Quarter_" + FakerData.getCourseName());
    test(`Create certification with Anniversary Date - Enrollment Date, Period Range Quarter`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create certification with Anniversary Date - Enrollment Date, Period Range Quarter` },
            { type: `Test Description`, description: `Create certification with expiry based on Enrollment Date, Period Range, Quarter, 1 year` }
        );

        const newData = {
            TP076: titleAnniversaryDate
        };
