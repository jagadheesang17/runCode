...
test.describe(`Verify_that_the_learner_can_retake_the_assessment_after_failing_it_in_the_first_attempt_for_a_section_level_assessment`, async () => {
    ...
    test(`Creation of Pre-Assessment`, async ({ adminHome, SurveyAssessment }) => {
        ...
        async function createQuestion(questionType: any) {
            ...
            await SurveyAssessment.enterRightAnswerFeedback("wrongAnswerFeedback");
            await SurveyAssessment.enterWrongAnswerFeedback("rightanswerFeedback");
    ...
    test(`Verify that the learner can retake the assessment after failing it in the first attempt for a with section level assessment and also with feedback `, async ({ learnerHome, catalog }) => {
        ...
        await catalog.tpPreAssessmentLaunch();
        async function assessmentSection1RightAnswer() {
            await catalog.writeContent();
        }
        await assessmentSection1RightAnswer();
        await catalog.clickSection2();
