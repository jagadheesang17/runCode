...
test.describe(`Verify_that_the_learner_can_retake_the_assessment_after_failing_it_in_the_first_attempt_for_a_without_section_level_assessment`, async () => {
    ...
    test(`Learninpath Creation with pre and post assessment attached`, async ({ adminHome, learningPath, createCourse }) => {
    ...
    test(`Verify the Learning path status moves to incomplete when the learner fails the post assessment after failing it in the first attempt for a without section level assessment and also with feedback`, async ({ learnerHome, catalog, dashboard }) => {
