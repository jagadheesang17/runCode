...
test(`Attaching the created survey with consider for completion to the learning path`, async ({ adminHome, learningPath, createCourse,createUser }) => {
    test.info().annotations.push(
        { type: `Author`, description: `Jagadish` },
 ...
 test(`Verify the course status after the learner completes the course when Consider for Completion is enabled for the survey` , async ({ learnerHome, catalog }) => {
