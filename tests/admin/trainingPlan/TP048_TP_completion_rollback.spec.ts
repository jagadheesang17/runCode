...
test(`Completing the main and equivalence course as a learner`, async ({ learnerHome, catalog }) => {
 ...
 test(`Creating learning path with main course attached`, async ({ adminHome, learningPath, createCourse, createUser }) => {
        test.info().annotations.push(
...
test(`Verify the TP status when the learner already completed the main course by standalone which is attached to the tp`, async ({ learnerHome, catalog }) => {
    test.info().annotations.push(
        { type: `Author`, description: `Tamilvanan` },
        { type: `TestCase`, description: `Verifying learner side that equivalence functionality works correctly` },
        { type: `Test Description`, description: `Verifying learner side that equivalence functionality works correctly` }
    ...
    // await catalog.clickMoreonCourse("Neural System Program");
