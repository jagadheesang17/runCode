...
test.describe(`TP048_Verify_lp_status_is_changed_to_incomplete_with_days_from_hire.spec.ts`, async () => {
    ...
    let title = ("LP_incomplete_days_from_hire" + " " + FakerData.getCourseName());

    test(`Create Learning Path with Days from Hire and incomplete`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create Learning Path with Days from Hire` },
            { type: `Test Description`, description: `Create learning path with Complete by Days from Hire and post complete by incomplete` }
        )
    ...
    test(`Verify LP status changed to Incomplete`, async ({ learnerHome, dashboard,catalog }) => {
        test.info().annotations.push(
            ...
            { type: `Test Description`, description: `Verify that learning path status is changed to incomplete after days from hire expired` }
        );
