...
test.describe(`TPC079_Verify_compliance_certification_expiration_with_anniversary_completion_date_fixed`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_Anniversary_" + FakerData.getCourseName());
    test(`Create compliance certification with Anniversary Date - Completion Date, Fixed Date`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Anniversary Date - Completion Date, Fixed Date` },
            { type: `Test Description`, description: `Create compliance certification with expiry based on Completion Date, Fixed Date, 1 year` }
        );

        const newData = {
            TPC079: titleAnniversaryDate
        };
