...
test.describe(`TPC083_Verify_compliance_certification_expiration_with_anniversary_hire_date_period_month`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_HireDate_Month_" + FakerData.getCourseName());
    test(`Create compliance certification with Anniversary Date - Hire Date, Period Range Month`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Anniversary Date - Hire Date, Period Range Month` },
            { type: `Test Description`, description: `Create compliance certification with expiry based on Hire Date, Period Range, Month, 1 year` }
        );

        const newData = {
            TPC083: titleAnniversaryDate
        };
