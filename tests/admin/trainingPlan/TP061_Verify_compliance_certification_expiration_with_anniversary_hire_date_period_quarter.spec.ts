...
test.describe(`TPC084_Verify_compliance_certification_expiration_with_anniversary_hire_date_period_quarter`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_HireDate_Quarter_" + FakerData.getCourseName());
    test(`Create compliance certification with Anniversary Date - Hire Date, Period Range Quarter`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Anniversary Date - Hire Date, Period Range Quarter` },
            { type: `Test Description`, description: `Create compliance certification with expiry based on Hire Date, Period Range, Quarter, 1 year` }
        );

        const newData = {
            TPC084: titleAnniversaryDate
        };
