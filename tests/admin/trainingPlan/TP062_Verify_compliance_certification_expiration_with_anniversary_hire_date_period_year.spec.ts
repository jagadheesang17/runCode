...
test.describe(`TPC085_Verify_compliance_certification_expiration_with_anniversary_hire_date_period_year`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_HireDate_Year_" + FakerData.getCourseName());
    test(`Create compliance certification with Anniversary Date - Hire Date, Period Range Year`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Anniversary Date - Hire Date, Period Range Year` },
            { type: `Test Description`, description: `Create compliance certification with expiry based on Hire Date, Period Range, Year, 1 year` }
        );

        const newData = {
            TPC085: titleAnniversaryDate
        };
