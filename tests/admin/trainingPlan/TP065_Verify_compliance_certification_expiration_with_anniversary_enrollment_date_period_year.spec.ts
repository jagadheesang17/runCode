...
test.describe(`TPC088_Verify_compliance_certification_expiration_with_anniversary_enrollment_date_period_year`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_EnrollmentDate_Year_" + FakerData.getCourseName());
    test(`Create compliance certification with Anniversary Date - Enrollment Date, Period Range Year`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Anniversary Date - Enrollment Date, Period Range Year` },
            { type: `Test Description`, description: `Create compliance certification with expiry based on Enrollment Date, Period Range, Year, 1 year` }
        );

        const newData = {
            TPC088: titleAnniversaryDate
        };
