...
test.describe(`TP090_Verify_compliance_certification_recertification_with_anniversary_birth_date_month_and_days_from_enrollment`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_RECERT_BirthDate_Month_DaysFromEnroll_" + FakerData.getCourseName());
    test(`Create compliance certification with Birth Date Month and Recertification Complete By Days from enrollment`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Birth Date Month expiry and Days from enrollment rule` },
            { type: `Test Description`, description: `Create compliance certification with Birth Date Period Month expiry and recertification complete by Days from enrollment` }
        );

        const newData = {
            TP090: titleAnniversaryDate
        };
        ...
        console.log(`🔄 Setting expiration with Anniversary Date - Birth Date, Period Range, Month`);
