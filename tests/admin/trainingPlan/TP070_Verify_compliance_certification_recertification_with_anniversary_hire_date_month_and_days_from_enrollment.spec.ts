...
test.describe(`TP093_Verify_compliance_certification_recertification_with_anniversary_hire_date_month_and_days_from_enrollment`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_RECERT_HireDate_Month_DaysFromEnroll_" + FakerData.getCourseName());
    test(`Create compliance certification with Hire Date Month and Recertification Complete By Days from enrollment`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Hire Date Month expiry and Days from enrollment rule` },
            { type: `Test Description`, description: `Create compliance certification with Hire Date Period Month expiry and recertification complete by Days from enrollment` }
        );

        const newData = {
            TP093: titleAnniversaryDate
        };
        ...
        console.log(`🔄 Setting expiration with Anniversary Date - Hire Date, Period Range, Month`);
