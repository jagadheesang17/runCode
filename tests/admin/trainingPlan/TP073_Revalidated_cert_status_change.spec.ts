...
test.describe(`Verify_that_when_the_learner_completes_the_revalidated_certification_the_status_changes_from_Completed_to_Expired`, async () => {
    ...
    test(`Re-validate the certification and complete it as a learner`, async ({ adminHome, enrollHome, catalog, editCourse, learnerHome }) => {
        ...
        await learnerHome.learnerLogin("LEARNERUSERNAME", "LeanrerPortal");
        await learnerHome.clickAlertIcon();
        await learnerHome.clickShowAll();
    ...
    test(`Verify the Certification status changed from completed to expired when the learner complete the revalidated certification`, async ({ adminHome, enrollHome, catalog, editCourse, learnerHome }) => {
               // Verify previous completed entry is now expired
      ...
      await adminHome.loadAndLogin("CUSTOMERADMIN1")
        ...
        await enrollHome.clickViewLearner();
        await enrollHome.enterSearchUser(credentials.LEARNERUSERNAME.username);
        await enrollHome.verifyPreviousEntryAsExpired();
    })
})