...
test.describe(`TP096_Verify_compliance_certification_recertification_with_anniversary_enrollment_date_month_and_days_from_enrollment`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_RECERT_EnrollDate_Month_DaysFromEnroll_" + FakerData.getCourseName());
    test(`Create compliance certification with Enrollment Date Month and Recertification Complete By Days from enrollment`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Enrollment Date Month expiry and Days from enrollment rule` },
            { type: `Test Description`, description: `Create compliance certification with Enrollment Date Period Month expiry and recertification complete by Days from enrollment` }
        );

        const newData = {
            TP096: titleAnniversaryDate
        };
        ...
        console.log(`🔄 Setting expiration with Anniversary Date - Enrollment Date, Period Range, Month`);
        ...
        console.log(`🔄 Setting recertification complete by rule: Days from enrollment`);
        await learningPath.clickAndSelectRecertCompleteByRule("Days from enrollment");
