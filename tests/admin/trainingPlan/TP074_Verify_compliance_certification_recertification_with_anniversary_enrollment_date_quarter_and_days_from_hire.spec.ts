...
test.describe(`TP097_Verify_compliance_certification_recertification_with_anniversary_enrollment_date_quarter_and_days_from_hire`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_RECERT_EnrollDate_Quarter_DaysFromHire_" + FakerData.getCourseName());
    test(`Create compliance certification with Enrollment Date Quarter and Recertification Complete By Days from hire`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Enrollment Date Quarter expiry and Days from hire rule` },
            { type: `Test Description`, description: `Create compliance certification with Enrollment Date Period Quarter expiry and recertification complete by Days from hire` }
        );

        const newData = {
            TP097: titleAnniversaryDate
        };
