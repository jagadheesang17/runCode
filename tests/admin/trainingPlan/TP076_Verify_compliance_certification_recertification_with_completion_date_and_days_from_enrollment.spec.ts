...
test.describe(`TP099_Verify_compliance_certification_recertification_with_completion_date_and_days_from_enrollment`, async () => {
    ...
    const titleCompletionDate = ("COMPL_CERT_RECERT_CompDate_DaysFromEnroll_" + FakerData.getCourseName());
    test(`Create compliance certification with Completion Date and Recertification Complete By Days from enrollment`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Completion Date expiry and Days from enrollment rule` },
            { type: `Test Description`, description: `Create compliance certification with Completion Date (1 Day) expiry and recertification complete by Days from enrollment` }
        );

        const newData = {
            TP099: titleCompletionDate
        };
