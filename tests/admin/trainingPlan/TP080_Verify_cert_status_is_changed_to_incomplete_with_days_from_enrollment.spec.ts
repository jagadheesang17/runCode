...
test.describe(`TP062_Verify_cert_status_is_changed_to_incomplete_with_days_from_enrollment.spec.ts`, async () => {
    ...
    let title = ("CERT_incomplete_days_from_enrollment" + " " + FakerData.getCourseName());

    test(`Create Certification with Days from Enrollment and incomplete`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create Certification with Days from Enrollment` },
            { type: `Test Description`, description: `Create certification with Complete by Days from Enrollment and post complete by incomplete` }
        )
