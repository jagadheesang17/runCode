...
test.describe(`TPC089_Verify_compliance_certification_recertification_expiry_with_anniversary_completion_date_fixed`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_RECERT_CompletionDate_Fixed_" + FakerData.getCourseName());
    test(`Create compliance certification with Anniversary Date and Recertification expiry`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Anniversary Date and Recertification expiry` },
            { type: `Test Description`, description: `Create compliance certification with expiry based on Completion Date Fixed and recertification expiry` }
        );

        const newData = {
            TPC089: titleAnniversaryDate
        };
        ...
        console.log(`🔄 Setting expiration with Anniversary Date - Completion Date, Fixed Date`);
        ...
        console.log(`🔄 Setting recertification expiry with Anniversary Date - Completion Date, Fixed Date`);
        await learningPath.clickReCertExpiresDropdown();
        await learningPath.clickReCertExpiresButtonWithType("Anniversary Date", {
            anniversaryType: "Completion",
            anniversaryRange: "Fixed Date",
            afterYears: "1"
        });
