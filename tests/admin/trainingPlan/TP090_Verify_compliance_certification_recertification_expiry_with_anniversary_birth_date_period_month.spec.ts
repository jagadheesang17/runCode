...
test.describe(`TPC090_Verify_compliance_certification_recertification_expiry_with_anniversary_birth_date_period_month`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_RECERT_BirthDate_Month_" + FakerData.getCourseName());
    test(`Create compliance certification with Anniversary Date Birth Date Month and Recertification expiry`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Anniversary Date Birth Date Month and Recertification expiry` },
            { type: `Test Description`, description: `Create compliance certification with expiry based on Birth Date, Period Range Month and recertification expiry` }
        );

        const newData = {
            TPC090: titleAnniversaryDate
        };
        ...
        console.log(`🔄 Setting recertification expiry with Anniversary Date - Birth Date, Period Range, Month`);
