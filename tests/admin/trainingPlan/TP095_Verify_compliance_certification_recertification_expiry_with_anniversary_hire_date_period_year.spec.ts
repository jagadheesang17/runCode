...
test.describe(`TPC095_Verify_compliance_certification_recertification_expiry_with_anniversary_hire_date_period_year`, async () => {
    ...
    const titleAnniversaryDate = ("COMPL_CERT_RECERT_HireDate_Year_" + FakerData.getCourseName());
    test(`Create compliance certification with Anniversary Date Hire Date Year and Recertification expiry`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Anniversary Date Hire Date Year and Recertification expiry` },
            { type: `Test Description`, description: `Create compliance certification with expiry based on Hire Date, Period Range Year and recertification expiry` }
        );

        const newData = {
            TPC095: titleAnniversaryDate
        };
        ...
        console.log(`🔄 Setting recertification expiry with Anniversary Date - Hire Date, Period Range, Year`);
