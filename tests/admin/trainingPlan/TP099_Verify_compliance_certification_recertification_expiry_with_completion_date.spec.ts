...
test.describe(`TPC099_Verify_compliance_certification_recertification_expiry_with_completion_date`, async () => {
    ...
    const titleCompletionDate = ("COMPL_CERT_RECERT_CompletionDate_" + FakerData.getCourseName());
    test(`Create compliance certification with Completion Date and Recertification expiry`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create compliance certification with Completion Date and Recertification expiry` },
            { type: `Test Description`, description: `Create compliance certification with expiry based on Completion Date (1 Day) and recertification expiry` }
        );

        const newData = {
            TPC099: titleCompletionDate
        };
