...
test.describe(`TP069_Verify_certification_expiration_with_anniversary_birth_date_period_month`, async () => {
    ...
    const titleAnniversaryDate = ("CERT_BirthDate_Month_" + FakerData.getCourseName());
    test(`Create certification with Anniversary Date - Birth Date, Period Range Month`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create certification with Anniversary Date - Birth Date, Period Range Month` },
            { type: `Test Description`, description: `Create certification with expiry based on Birth Date, Period Range, Month, 1 year` }
        );

        const newData = {
            TP069: titleAnniversaryDate
        };
