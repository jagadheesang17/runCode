...
test.describe(`TP072_Verify_certification_expiration_with_anniversary_hire_date_period_month`, async () => {
    ...
    const titleAnniversaryDate = ("CERT_HireDate_Month_" + FakerData.getCourseName());
    test(`Create certification with Anniversary Date - Hire Date, Period Range Month`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create certification with Anniversary Date - Hire Date, Period Range Month` },
            { type: `Test Description`, description: `Create certification with expiry based on Hire Date, Period Range, Month, 1 year` }
        );

        const newData = {
            TP072: titleAnniversaryDate
        };
