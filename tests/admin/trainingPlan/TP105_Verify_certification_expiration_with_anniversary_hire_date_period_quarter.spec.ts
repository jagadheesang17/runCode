...
test.describe(`TP073_Verify_certification_expiration_with_anniversary_hire_date_period_quarter`, async () => {
    ...
    const titleAnniversaryDate = ("CERT_HireDate_Quarter_" + FakerData.getCourseName());
    test(`Create certification with Anniversary Date - Hire Date, Period Range Quarter`, async ({ adminHome, learningPath, createCourse }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
            { type: `TestCase`, description: `Create certification with Anniversary Date - Hire Date, Period Range Quarter` },
            { type: `Test Description`, description: `Create certification with expiry based on Hire Date, Period Range, Quarter, 1 year` }
        );

        const newData = {
            TP073: titleAnniversaryDate
        };
