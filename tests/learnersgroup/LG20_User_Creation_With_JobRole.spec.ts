...
let groupTitle = FakerData.getFirstName() + "_JobRoleGroup";

// Store fresh metadata for this test
let freshMetadata: any;
test.describe(`User Creation with Job Role Data Tests`, () => {
    test.describe.configure({ mode: "serial" });

    test.beforeAll('Create Fresh Metadata', async () => {
        // Create fresh metadata for this test file
        freshMetadata = await TestMetadataManager.createFreshMetadata('User_Creation_With_JobRole.spec.ts');
    ...
    test(`Verify user creation with all metadata attributes using fresh metadata created via API`, async ({ adminHome, createUser }) => {
        test.info().annotations.push(
            ...
            { type: `TestCase`, description: `UC005 - User Creation with Job Role Data` },
            { type: `Test Description`, description: `Create a user with job role, department, employment type, user type, and job title using fresh metadata created via API` }
        );
    ...
    test(`Create learner group with same job role from user data`, async ({ adminHome, learnerGroup }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Jagadish` },
            { type: `TestCase`, description: `UC006 - Learner Group Creation with User Job Role Data` },
            { type: `Test Description`, description: `Create a learner group using the same job role data from the user creation test` }
        );
